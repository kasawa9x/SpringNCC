package com.example.demospringncc.controller;

import com.example.demospringncc.GlobalData.GlobalData;
import com.example.demospringncc.model.Product;
import com.example.demospringncc.service.ProductService;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Controller;
import org.springframework.ui.Model;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;

@Controller
public class CartController {
    @Autowired
    ProductService productService;
    @GetMapping("/addToCart/{id}")
    public String addToCart(@PathVariable int id){
        GlobalData.cart.add(productService.getProductyById(id).get());
        return "redirect:/shop";
    }
    @GetMapping("/cart")
    public String cartGet(Model model){
        final long shipment = 300 ;
        model.addAttribute("cartCount", GlobalData.cart.size());
        model.addAttribute("total",GlobalData.cart.stream().mapToLong(Product::getPrice).sum());
        model.addAttribute("total1",GlobalData.cart.stream().mapToLong(Product::getPrice).sum()+shipment);

        model.addAttribute("cart",GlobalData.cart);
        return "cart";
    }
    @GetMapping("/cart/removeItem/{index}")
    public String cartItemRemove(@PathVariable int index ){
        GlobalData.cart.remove(index);
        return "redirect:/cart";

    }
    @GetMapping("/checkout")
    public String checkout(Model model){
        final long shipment = 300 ;
        model.addAttribute("total",GlobalData.cart.stream().mapToLong(Product::getPrice).sum());
        model.addAttribute("total1",GlobalData.cart.stream().mapToLong(Product::getPrice).sum()+shipment);
        return "checkout";
    }
}
